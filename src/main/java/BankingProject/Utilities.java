...
//			System.out.println("The name is: " + name);
    ...
    protected void deposit(int position){
        ...
        SessionData.I().saveLocalData();
        print("Your money deposited succesfully\n"
                + "Deposite money : $"+amount+"\n"
     ...
     * @param position  Get position of user bank data object
     */
    protected void transferFunds(int position){
        ...
        if(option>2) {
            ...
            // validation on Reciever account number
            ...
            int recieverPosition=0;

            for(int i=0; i<SessionData.I().localData.accountList.size();i++) {

                if(SessionData.I().localData.accountList.get(i).getAccountNumber()==accountNumber ) {
                    if ( SessionData.I().localData.accountList.get(position).getAccountNumber()== accountNumber ) {
                    ...
                    print("\nPlease verify the reciever's details\n"
                    ...
                    if(yesOrNo.equalsIgnoreCase("y")) {
                        ...
                        if(currentBalance>=amount){
                            ...
                            SessionData.I().localData.accountList.get(position).setBalance(currentBalance);
                            //							if(option==1) {
                            double currentRecieverBalance=SessionData.I().localData.accountList.get(recieverPosition).getBalance();
                            currentRecieverBalance=currentRecieverBalance+amount;
                            SessionData.I().localData.accountList.get(position).getMiniStatementList().add(miniStatementData(amount,Constants.DEBIT));
                            SessionData.I().localData.accountList.get(recieverPosition).setBalance(currentRecieverBalance);
                            SessionData.I().localData.accountList.get(recieverPosition).getMiniStatementList().add(miniStatementData(amount,Constants.CREDIT));
                            //							}else {
                            //								SessionData.I().localData.accountList.get(position).getMiniStatementList().add(miniStatementData(amount,Constants.DEBIT));
                            //							}
                            SessionData.I().saveLocalData();
